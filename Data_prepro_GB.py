...
season_range = list(range(2,3))
day_range = list(range(10,11))
...
for temp_season_number in season_range:
    ...
    for temp_day_number in day_range:
        dir_path = "RawData/GlobalServer/GBSeason"+str(temp_season_number)+"/day"+str(temp_day_number)+"/"
        ...
        for csv_path in csv_list:
            ...
            #world, job이 이름이 아니라 ID값으로 되어있음. for 문에서 돌리고 있는 로컬변수를 칼럼 값에 replace하기.
            ...
            temp_df = temp_df[['Day_Number','World','Job','Points',"Rank","Name"]] #칼럼 순서 맞추기
        ...
        #빠져나와서 인데스 재정령하고 저장하기.
        save_df.reset_index(drop=True,inplace=True)
        save_path = "DataPreprocessing/GlobalServer/Season"+ str(temp_season_number)+"/"
        save_file_name = "GB_Season" + str(temp_season_number) + "_" + "Day"+str(temp_day_number)+".csv"
        save_df.to_csv(save_path +save_file_name)
